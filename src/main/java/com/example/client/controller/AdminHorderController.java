...
public class AdminHorderController {
	...
	@RequestParam("notif_isi") String notif_isi
		...
		return "redirect:/adminlist_horder.html?success";
	}
	...
	@RequestMapping("/print_inv.html/{horder_id}")
	public String cetakdonasi(Model model, @PathVariable(name = "horder_id") int horder_id) {
		List listprintinv = horderrepo.listprint(horder_id+"");
		model.addAttribute("listprintinv", listprintinv);
		
		List listprintinvheader = horderrepo.listprintheader(horder_id+"");
		model.addAttribute("listprintinvheader", listprintinvheader);
